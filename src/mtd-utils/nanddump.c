 ...
 *
 *  Modifications by Ken Steele <ken@chumby.com>:
 *       - modified bad block detection method
 *       - modified bad block omission to append a block of 0xFF bytes
 *         to the end of the output; useful for comparing partition
 *         checksums regardless of bad blocks.
 */
...
unsigned char oobbuffer[16];
unsigned char badBlockBuf[16384];
...
int main(int argc, char **argv)
{
	...
	for (ofs = start_addr; ofs < end_addr ; ofs+=bs) {
		...
		if (blockstart != (ofs & (~meminfo.erasesize + 1))) {
			...
			// Ken's code - not sure why this was originally needed, may be
			// ironforge-specific...

            //check for bad block
            struct mtd_oob_buf oob1 = {0, 16, oobbuffer};

            oob1.start = ofs & 0xFFFFC000;

            // Read OOB data and exit on failure
            if( ioctl( fd, MEMREADOOB, &oob1 ) != 0 )
            {
    ...
    if( omitoob && omitbad )
    {
        // append 0xFF blocks to the end of the output so the total
        // size matches that of a partition with no bad blocks

        for ( i = 0; i < bbCount / 32; ++i )
        {
            memset( badBlockBuf, 0xFF, meminfo.erasesize );
            write( ofd, badBlockBuf, meminfo.erasesize );
        }
    }

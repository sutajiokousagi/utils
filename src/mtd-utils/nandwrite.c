...
int main(int argc, char **argv)
{
	...
	while (imglen && (mtdoffset < meminfo.size)) {
		...
		actual_written = pwrite(fd, writebuf, meminfo.writesize, mtdoffset);
		if (actual_written != meminfo.writesize) {
			int rewind_blocks;
			off_t rewind_bytes;
			erase_info_t erase;

			printf( "pwrite failed, bytes requested/written = %d/%d, errno = %d (%s) offset %x\n",
				meminfo.writesize, actual_written, errno, strerror(errno), mtdoffset );

...
static void updateProgressBar( int perc, int bad );
...
void updateProgressBar( int perc, int bad )
{
    int barValue;
    int r, g, b;
...
int main (int argc, char *argv[])
{
	...
	for (erase.start = 0; erase.start < meminfo.size; erase.start += meminfo.erasesize)
	{
		if (bbtest)
		{
            ...
            struct mtd_oob_buf oobRead = {0, 16, oobbuf};

            oobRead.start = erase.start;

    		// Read OOB data and exit on failure
    		if( ioctl( fd, MEMREADOOB, &oobRead ) != 0 )
    		{
            ...
            if( oobbuf[5] != 0xFF )
            {
                // found a bad block
 				if (!quiet)
				{
                    printf( "\nSkipping bad block %d [0x%08x]\n", (int)(erase.start / 0x4000), erase.start );
 				}
		...
		if (!quiet)
		{
            if( (int)(erase.start / 0x4000) % 10 == 0 )
            {
				updateProgressBar( (unsigned long long)erase.start * 100 / meminfo.size, 0 );
				printf( "\rErasing %d Kibyte @ %x -- %2llu %% complete.",
					meminfo.erasesize / 1024, erase.start, (unsigned long long)erase.start * 100 / meminfo.size);
    		}
		}
		fflush(stdout);
...
void process_options (int argc, char *argv[])
{
	...
	for (;;) {
		int option_index = 0;
		static const char *short_options = "jqn";
...
void display_help (void)
{
	printf("Usage: %s [OPTION] MTD_DEVICE\n"
			...
			"  -n, --noscan   don't scan for bad blocks\n"

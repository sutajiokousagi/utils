...
int main(int argc, char **argv) {
    return Test_Jpeg_Display(argc, argv);
}
int Test_Jpeg_Display(int argc, char **argv)
{
	...
	char *input_file = argc > 1 ? argv[1] : JPEG_INPUT_FILE;
    ...
    int out_height = LCD_HEIGHT;
    int out_width = LCD_WIDTH;
    int alt_height = height*out_width/width;
    if (alt_height>height) {
        out_width = width*out_height/height;
    } else {
        out_height = alt_height;
    }
	...
	pp_param.DstStartX		= (LCD_WIDTH-out_width)/2;
	pp_param.DstStartY		= (LCD_HEIGHT-out_height)/2;
	pp_param.DstFullWidth	= LCD_WIDTH;
	pp_param.DstFullHeight	= LCD_HEIGHT;
	pp_param.DstWidth		= out_width;
	pp_param.DstHeight		= out_height;
    ...
    // bzero is necessary as the output image may be smaller than the full
    // screen size.
    bzero(pp_out_buf, LCD_WIDTH*LCD_HEIGHT*2);

...
//#define LOG_MSG(target, prog, ...) fprintf(stderr, __VA_ARGS__)
#define LOG_MSG(target, prog, ...)


//////////////////////////////////////////////////////////////////////////////////////
// Definition																		//
//////////////////////////////////////////////////////////////////////////////////////
#define JPG_DRIVER_NAME		"/dev/misc/s3c-jpg"
#define POST_DRIVER_NAME	"/dev/misc/s3c-pp"
...
JPEG_ERRORTYPE SsbSipJPEGDecodeExe(int dev_fd)
{
	...
	if(jCtx->decParam->decType == JPG_MAIN){
		result = ioctl(dev_fd, IOCTL_JPG_DECODE, jCtx->decParam);
...
static JPEG_ERRORTYPE makeThumbImage(int dev_fd)
{
	...
	LOG_MSG(LOG_TRACE, "makeThumbImage", "jCtx->OutThumbBuf(addr : 0x%p)\n", jCtx->OutThumbBuf);
...
Lable:
	...
	if(Exif){
		...
		LOG_MSG(LOG_TRACE, "SsbSipJPEGEncodeExe", "jCtx->OutBuf(addr : 0x%p)\n", jCtx->OutBuf);
...
void *SsbSipJPEGGetDecodeOutBuf (int dev_fd, long *size)
{
	...
	LOG_MSG(LOG_TRACE, "SsbSipJPEGGetDecodeOutBuf", "DecodeOutBuf : 0x%p  size : %d\n", jCtx->OutBuf, jCtx->decParam->dataSize);
...
void *SsbSipJPEGGetEncodeInBuf(int dev_fd, long size)
{
	...
	LOG_MSG(LOG_TRACE, "SsbSipJPEGEncodeInBuf", "EncodeInBuf : 0x%p  size :%d\n", jCtx->InBuf, jCtx->encParam->dataSize);
	return (jCtx->InBuf);
}
...
void *SsbSipJPEGGetEncodeOutBuf (int dev_fd, long *size)
{
	*size = jCtx->encParam->fileSize;
	...
	LOG_MSG(LOG_TRACE, "SsbSipJPEGGetEncodeOutBuf", "jCtx->OutBuf(addr : 0x%p  size : %ld)\n", jCtx->OutBuf, *size);

...
int Test_Display_4_Windows(int argc, char **argv)
{
	...
	pp_param1.DstFullWidth	= 400;		// destination width
	pp_param1.DstFullHeight	= 240;		// destination height
	...
	osd_info_to_driver1.Width		= 400;	// display width
	osd_info_to_driver1.Height		= 240;	// display height
	...
	while(!last_frame1 || !last_frame2 || !last_frame3 || !last_frame4)
	{
		...
		if (last_frame1 != 1) {
			...
			ioctl(pp_fd1, PPROC_SET_PARAMS, &pp_param1);
			ioctl(pp_fd1, PPROC_START);		
		...
		if (last_frame2 != 1) {
			...
			ioctl(pp_fd2, PPROC_SET_PARAMS, &pp_param2);
			ioctl(pp_fd2, PPROC_START);		
		...
		if (last_frame3 != 1) {
			...
			ioctl(pp_fd3, PPROC_SET_PARAMS, &pp_param3);
			ioctl(pp_fd3, PPROC_START);	 	
		...
		if (last_frame4 != 1) {
			...
			ioctl(pp_fd4, PPROC_SET_PARAMS, &pp_param4);
			ioctl(pp_fd4, PPROC_START);	

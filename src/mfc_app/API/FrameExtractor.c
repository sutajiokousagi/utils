...
static int next_delimiter(FRAMEX_CTX *pCTX, FILE *fpin, unsigned char *outbuf, const int outbuf_size, int *n_fill)
{
	...
	// delimiter와 동일한 octet stream을 찾아낸다.
	for (l=0; l<pCTX->delim_leng; l++) {
		if (q_size == 0 || l == q_size) {
			...
			if (r != 1)	// EOF를 만난경우..
				break;
			offset++;
			if (outbuf != NULL) {	// outbuf에 읽어낸 값을 복사함.
...
int FrameExtractorPeek(FRAMEX_CTX *pCTX, FRAMEX_IN in, unsigned char peekbuf[], int peek_size, int *n_fill)
{
	...
	if (pCTX->in_type == FRAMEX_IN_TYPE_FILE) {
		*n_fill = fread(peekbuf, 1, peek_size, (FILE *) in);
		if (*n_fill > 0) {
			fseek((FILE *) in, -peek_size, SEEK_CUR);
			return FRAMEX_OK;
		}
		return FRAMEX_ERR_EOS;
	}
	else {
		strm_ptr = (FRAMEX_STRM_PTR *) in;

		if ((int) (strm_ptr->p_cur + peek_size) > (int) strm_ptr->p_end)
			peek_size = (int) strm_ptr->p_end - (int) strm_ptr->p_cur;

		memcpy(peekbuf, strm_ptr->p_cur, peek_size);
		*n_fill = peek_size;

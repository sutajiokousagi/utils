...
static log2_max_frame_num_minus4 = 0;
...
{
	int            bit_offset;
//	unsigned int   first_mb, slice_type, pic_parameter_set_id, frame_num;
                            ...
                            H264_CONFIG_DATA *conf_data)
{
	...
	if (includeIframe == 1) {
		nFrameSize = NextFrameH264(pFrameExCtx, fp, buf + nStreamSize, buf_size - nStreamSize, &coding_type);
...
int NextFrameH264(FRAMEX_CTX  *pFrameExCtx, void *fp, unsigned char buf[], int buf_size, unsigned int *coding_type)
{
	int                i;
	int                ret;
	unsigned char      nal_first_8bytes[10];
	unsigned char      nal_type;
	int                nStreamSize, nFrameSize;
	...
	for (i=0; i<30; i++) {
		...
		ret = FrameExtractorPeek(pFrameExCtx, fp, nal_first_8bytes, sizeof(nal_first_8bytes), (int *)&nFrameSize);
		nal_type = NAL_UNIT_TYPE_TYPE(nal_first_8bytes[4]);
	...
	for (i=0; i<15; i++) {
		...
		if (nal_type != NAL_UNIT_TYPE_TYPE(nal_first_8bytes[4]))
			break;


		// first_mb와 frame_num을 비교한다.
		get_h264_slice_info(nal_first_8bytes + 5, &h264_slice_info_next);
		if (h264_slice_info.frame_num != h264_slice_info_next.frame_num)
			break;
		if (h264_slice_info.first_mb == h264_slice_info_next.first_mb)
			break;

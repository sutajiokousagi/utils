/*
 * $Id$ 
 * test_sound_output.cpp - map EVT_ABS on a HID device to PCM sample files to blast to stdout
 * (for use with aplaypipe - e.g. test_sound_output /tmp/sound.cfg | aplaypipe
**/
...
// Send a sample file to stdout
void
send_str( const char *s, bool doFlush = true )
{
	// Parse multi-action lines
	if (strchr( s, ';' ))
	{
		fprintf( stderr, "Multi-action line: {%s\n", s );
		char *scopy = strdup( s );
		if (!scopy) return;
		for (s = strtok( scopy, ";" ); s; s = strtok( NULL, ";" ))
		{
			send_str( s, false );
		}
		free( scopy );
		fprintf( stderr, "}\n" );
		// Flush after entire group unless prohibited
		if (doFlush)
		{
			fflush( stdout );
		}
	...
	if (!strncmp( s, "silence(", 8 ))
	{
		int samples_silence;
		if (sscanf( s, "silence(%d)", &samples_silence ) >= 1)
		{
			fprintf( stderr, "Sending %d samples of silence (%d bytes)\n", samples_silence, samples_silence * 4 );
			char buff[4096];
			memset( buff, 0, sizeof(buff) );
			while (samples_silence > 0)
			{
				int write_samples = samples_silence > 1024 ? 1024 : samples_silence;
				if (fwrite( buff, 4 * sizeof(char), write_samples, stdout ) < write_samples)
				{
					fprintf( stderr, "Write failed, errno=%d (%s)\n", errno, strerror(errno) );
					break;
				}
				samples_silence -= write_samples;
			}
		}
		else
		{
			fprintf( stderr, "Failed to parse sample count from %s\n", s );
		}
		return;
	}
	...
	while ((bytes_read = fread( buff, sizeof(unsigned char), sizeof(buff), fIn )) > 0)
	{
		totalwritten += bytes_read;
		fwrite( buff, sizeof(unsigned char), bytes_read, stdout );
	}
	fprintf( stderr, "[%lu]\n", totalwritten );
	// Using fwrite, this should not make any difference...
	if (doFlush)
	{
		fflush( stdout );
	}
   ...
   while (!exitRequested && !g_quit)
	{
        ...
        {
			fprintf( stderr, "stdout not accepting output - output will block\n" );
			//break;
        }
        if (!inputReady)
        {
...
{
	fprintf( stderr, "test_sound_output v" VER_STR "\n" );
	if (argc < 3)
	{
	...
	// [t=type]	xmin	xmax	ymin	ymax	sample-file	[rept-count	[rept-delay]]
	// if first token is "immediate" action will be processed at parse time
	char buff[1024];
	while (fgets( buff, sizeof(buff), fKeymap ))
	{
		...
		else if (t1 && !strcasecmp( t1, "immediate" ))
		{
			// Ignore xmax, ymin, ymax
			t2 = strtok( NULL, " \t\r\n" );
			t3 = strtok( NULL, " \t\r\n" );
			t4 = strtok( NULL, " \t\r\n" );
			t5 = strtok( NULL, "\t\r\n" );
			if (t5)
			{
				fprintf( stderr, "Processing immediate value %s\n", t5 );
				send_str( t5 );
			}
			continue;
		}
